...
func (h *VenueHTTPHandler) logAdminAction(ctx context.Context, action string, venueID int, details map[string]any) {
	...
	h.adminLogger.Log(ctx, domain.AdminLog{
		...
		Action:      action,
		TargetType:  domain.LogTargetVenue,
		TargetID:    strconv.Itoa(venueID),
		Details:     details,
	})
}
