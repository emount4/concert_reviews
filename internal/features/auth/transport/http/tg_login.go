...
func (h *AuthHTTPHandler) Link(rw http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	log := core_logger.FromContext(ctx)
	responseHandler := core_http_response.NewHTTPResponseHandler(log, rw)
	...
	if err != nil {
		responseHandler.ErrorResponse(err, "failed to link tg")
		return
	}

	err = h.authService.Link(ctx, userID, req.InitData)
	if err != nil {
		responseHandler.ErrorResponse(err, "failed to link tg")
		return
	}
